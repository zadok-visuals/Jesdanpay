...
// until a real webhook fires against a live sandbox account.
export async function POST(request: Request) {
  const rawBody = await request.text();
  const admin = createAdminClient();

  let payload: { requestId?: string; id?: number; payoutStatus?: string; amount?: number };
  try {
  ...
  await admin.from("webhook_events").insert({
    provider: "klasha",
    event_type: payload.payoutStatus ?? "unknown",
